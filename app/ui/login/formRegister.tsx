...
export default function FormRegister() {
    ...
    return(<form className="w-full flex flex-col justify-center items-center mt-11" onSubmit={handleSubmit}>
        ...
        <div className="bg-white h-11 p-2 my-2 rounded-xl grid grid-cols-[1fr] grid-rows-1 items-center relative overflow-hidden">
            <div className="pt-2">
                ...
                value={formData.password} name="password" type={showPass1?"text":"password"} onChange={handleChange} id="pass" placeholder="." disabled={false} required></input>
                <label className="absolute left-2 top-0 duration-300" htmlFor="pass">New Password</label>
                <p className="absolute text-sm h-full top-0 right-0 bg-white p-2 flex items-center text-black font-bold cursor-default" onClick={() => setShowPass1(!showPass1)}>{showPass1?"Hide":"Show"}</p>
            </div>
        </div>
        <div className="bg-white h-11 p-2 my-2 rounded-xl grid grid-cols-[1fr] grid-rows-1 items-center relative overflow-hidden">
            <div className="pt-2">
                ...
                value={confirmPass} name="passConfirm" type={showPass2?"text":"password"} onChange={handleConfirm} id="pass2" placeholder="." disabled={false} required></input>
                <label className="absolute left-2 top-0 duration-300" htmlFor="pass2">Confirm Password</label>
                <p className="absolute text-sm h-full top-0 right-0 bg-white p-2 flex items-center text-black font-bold cursor-default" onClick={() => setShowPass2(!showPass2)}>{showPass2?"Hide":"Show"}</p>
            </div>
        </div>
