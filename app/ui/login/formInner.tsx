...
export default function FormLogin() {
    ...
    return(<form className="w-full flex flex-col justify-center items-center mt-11" onSubmit={handleSubmit}>
        ...
        <div className="bg-white h-11 p-2 my-2 rounded-xl grid grid-cols-[40px_1fr] grid-rows-1 items-center relative">
            <Image className="w-8 h-8" src="/Icons/user-login.svg" width="100" height="100" alt="Icon"></Image>
            <div className="pt-2">
                ...
                <label className="absolute left-12 top-0 duration-300" htmlFor="username">Username</label>
            </div>
        </div>
        <div className="bg-white h-11 p-2 rounded-xl grid grid-cols-[40px_1fr] grid-rows-1 items-center relative overflow-hidden">
            <Image className="w-7 h-7" src="/Icons/lock-login.svg" width="100" height="100" alt="Icon"></Image>
            <div className="pt-2">
                ...
                value={formData.password} name="password" type={showPass?"text":"password"} onChange={handleChange} id="pass" placeholder="." disabled={false} required></input>
                <label className="absolute left-12 top-0 duration-300" htmlFor="pass">Password</label>
                <p className="absolute text-sm h-full top-0 right-0 bg-white p-2 flex items-center text-black font-bold cursor-default" onClick={() => setShowPass(!showPass)}>{showPass?"Hide":"Show"}</p>
            </div>
        </div>
